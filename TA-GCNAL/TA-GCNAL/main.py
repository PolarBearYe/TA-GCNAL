...
def supervised_learning(args):
    ...
    train_parser.add_argument('--n_epoch', type=int, default=1000)
    ...
    train_parser.add_argument('--vit_pretrained_weights', type=str, default='./dino_deitsmall16_pretrain.pth')
...
def al_train(args, train_args, train_params, strategy_name):
    ...
    general_path = os.path.join(main_path,'%' +
                                ...
                                '_rounds' + str(15) + '_' + train_args.model + '_emb' + str(train_args.emb_size) +
...
def al_train_sub_experiment(args, train_args, train_params, strategy_name, general_path, seed):
    ...
    result_file = open(sub_path + '.csv', 'w',newline='')
    ...
    for rd in range(1, args.n_round + 1):
        ...
        strategy_name = 'GCNSampling'
        cls = globals()[strategy_name]
        strategy = cls(X_tr, Y_tr, idxs_lb, X_val, Y_val, model, args, device, writer)

        q_idxs1, embeddings, preds, probs, u_idxs, candidate_idxs = strategy.query(budget,n_bai_fen)
        del cls
        idxs_unlabeled =q_idxs1
        strategy_name = 'CDALSampling'
        cls = globals()[strategy_name]
        strategy = cls(X_tr, Y_tr, idxs_lb, X_val, Y_val, model, args, device, writer)
        q_idxs,embeddings, preds, probs, u_idxs, candidate_idxs = strategy.query(budget,idxs_unlabeled)
...
def get_query_diversity_uncertainty(embeddings, gt_y, p_y, probs, writer, rd):
    ...
    #.add_scalar('selection_statistics/gt_entropy', ent, rd)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_name', type=str, default='MNIST',choices=['MNIST', 'EMNIST',
                                                          'SVHN', 'CIFAR10','CIFAR10IM',
                                                          'CIFAR100', 'MiniImageNet',
                                                          'domain_net-real', 'mini_domain_net-real', 'tiny_domain_net-real',
                                                          'openml_6', 'openml_155'])
    parser.add_argument('--n_label', type=int, default=26, help='The number of distinct classes in the dataset.')
