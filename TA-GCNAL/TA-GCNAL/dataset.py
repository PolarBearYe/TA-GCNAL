...
def get_dataset(name, data_dir):
    ...
    if name == 'EMNIST':
        return get_MNISTIM(data_dir)
    elif name == 'SVHN':
        return get_SVHN(data_dir)
    elif name == 'CIFAR10':
        return get_CIFAR10(data_dir)
    elif name == 'CIFAR10IM':
        return get_CIFAR10IM(data_dir)
    elif name == 'CIFAR100':
...
def get_CIFAR10IM(data_dir):
    data_tr = datasets.CIFAR10(os.path.join(data_dir, 'CIFAR10'), train=True, download=True)
    data_te = datasets.CIFAR10(os.path.join(data_dir, 'CIFAR10'), train=False, download=True)

    X_te = data_te.data
    Y_te = torch.from_numpy(np.array(data_te.targets))
    targets = np.array(data_tr.targets)
    ...
    data_tr.targets = targets[imb_class_idx]
    data_tr.data = data_tr.data[imb_class_idx]
    X_tr = data_tr.data
    Y_tr = torch.from_numpy(np.array(data_tr.targets))
